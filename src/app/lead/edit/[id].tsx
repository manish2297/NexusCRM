...
export default function EditLeadScreen() {
  ...
  // Field states initialized to existing values
  const [name, setName] = useState(lead?.name || '');
  const [company, setCompany] = useState(lead?.company || '');
  const [email, setEmail] = useState(lead?.email || '');
  const [phone, setPhone] = useState(lead?.phone || '');
  const [status, setStatus] = useState<'New' | 'Contacted' | 'Proposal' | 'Won' | 'Lost'>(lead?.status || 'New');
  ...
  const handleUpdate = () => {
    ...
    // Notify & Back
    if (Platform.OS === 'web') {
      alert(`Lead ${name} updated successfully!`);
      router.back();
    } else {
      Alert.alert(
        'Changes Saved', 
        `Changes to lead "${name}" have been saved successfully.`,
  ...
  return (
    <ScrollView
      ...
      <View style={styles.mainWrapper}>
        ...
        <Text style={[styles.pageTitle, { color: theme.text }]}>Modify Lead Account</Text>
        <Text style={[styles.pageSub, { color: theme.textSecondary }]}>Updating records will automatically recalculate CRM pipelines and log changes in the audit trail.</Text>
